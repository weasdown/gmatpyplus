...
class ForceModel(GmatObject):
    def __init__(self, name: str = 'DefaultProp_ForceModel', central_body: str = 'Earth',
        ...
        if not drag:
            self.drag = False
        elif isinstance(drag, DragForce):
            self.drag = drag
            self.AddForce(self.drag)
        else:
            self.drag = DragForce(fm=self)  # create and use a default drag model
            self.AddForce(self.drag)
    ...
    class PrimaryBody:
        ...
        def __init__(self, fm: ForceModel, body: str = 'Earth',
            ...
            self._drag = drag if drag else DragForce(self._force_model)
